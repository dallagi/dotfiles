...
local function open_window()
  ...
  api.nvim_buf_set_option(buf, 'filetype', 'status-window')
  ...
  local title = " Some info "
  local left_padding = math.floor((win_width - string.len(title)) / 2)
  local right_padding = win_width - left_padding - string.len(title)
  local border_lines = { '╭' .. string.rep('─', left_padding) .. title .. string.rep('─', right_padding) .. '╮' }
...
local function set_mappings()
  ...
  for k,v in pairs(mappings) do
    api.nvim_buf_set_keymap(buf, 'n', k, ':lua require"status-window".'..v..'<cr>', {
