...
#+begin_src emacs-lisp
  ...
  (defun my/python-lsp-command ()
    "Use poetry to run pyright when possible, or run pyright directly instead"
    (let* ((poetry-check-result (call-process "poetry" nil nil nil "check"))
	   (in-poetry-project? (= 0 poetry-check-result)))
      (if in-poetry-project?
  ...
  (use-package eglot
    ...
    (add-to-list 'eglot-server-programs '((python-ts-mode python-mode) .
					  (lambda (x) "" (my/python-lsp-command))))
